...
def metashape_to_json(
    ...
    if sensor_type.count(sensor_type[0]) != len(sensor_type):
        raise ValueError(
            ...
            "nerfstudio does not support per-frame camera_model types, only one camera type can be used: frame, fisheye or spherical (perspective, fisheye or equirectangular)"
        )
    data = {}
